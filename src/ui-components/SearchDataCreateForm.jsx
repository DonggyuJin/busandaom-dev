...
export default function SearchDataCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setSearchAddressA(initialValues.searchAddressA);
    setSearchAddressB(initialValues.searchAddressB);
    setErrors({});
  };
  const validations = {
    searchAddressA: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(new SearchData(modelFields));
          if (onSuccess) {
      ...
      {...rest}
      {...getOverrideProps(overrides, "SearchDataCreateForm")}
    >
      <TextField
